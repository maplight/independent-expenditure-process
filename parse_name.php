...
function parse_ie_names()
{
    global $ie_db;

    $query = "SELECT * FROM independent_expenditure_filings_names_to_standardize";
    $result = $ie_db->prepare($query);
    $result->execute(array());
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        $filing_id = $row["ExpenditureFilingID"];
        $name = $row["TargetCandidateNamePreStandardization"];
        $parsed_name = parse_name($name);

        $query2 = "UPDATE independent_expenditure_filings_names_to_standardize SET name_prefix = ?, name_suffix = ?, first_name = ?, last_name = ?, middle_name = ?, nick_name = ?, gender = ? WHERE ExpenditureFilingID = ?";
        $result2 = $ie_db->prepare($query2);
